...
server.post("/lexicos/", function (req, res, next) {
  const error = validateLexico(req.body);
  if (error) {
    res.status(400).send(error);
  } else {
    req.body.slug = createSlug(req.body.title); // Generate a slug for new lexicos.
    next();
  }
});
...
function validateLexico(lexico) {
  if (!lexico.entry) return "Entry is required.";
  if (!lexico.type) return "type is required.";
  return "";
}
